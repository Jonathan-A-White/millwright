...
func newNudgeCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long: "nudge reads this host's claimed stories and, of each, whether it has run longer than\n" +
			...
			"(default 20). For each that holds, it prints one line, tab-separated: a key a caller can\n" +
			"damp a condition still holding by, and the clause itself. contrib/mail-notify is the only\n" +
			"caller: it composes the clauses into the one line it types into the Mayor's window, damped\n" +
			"to at most once a condition an hour.\n\n" +
